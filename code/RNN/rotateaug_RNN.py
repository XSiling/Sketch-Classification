...
def generate_rotate(X, Y, angle=45, por=0.3):
    b = torchvision.transforms.functional.rotate(X,angle)
    index = []
...
class mydataset(Dataset):
    def __init__(self,filename='ML-sketch/train_data.pkl',transform=torchvision.transforms.ToTensor()) :
        ...
        train_X_rotate1, train_Y_rotate1 = generate_rotate(self.dataset['X'],self.dataset['Y'],45)
        train_X_rotate2, train_Y_rotate2 = generate_rotate(self.dataset['X'],self.dataset['Y'],-45)
        
        self.dataset['X'] = torch.cat((self.dataset['X'], train_X_rotate1, train_X_rotate2), dim=0)
        self.dataset['Y'] = torch.cat((self.dataset['Y'], train_Y_rotate1, train_Y_rotate2),dim=0)
