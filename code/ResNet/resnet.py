...
class ResNet(nn.Module):
    ...
    def __init__(self, block: object, groups: object, num_classes=25) -> object:
        ...
        self.conv3_x = self._make_conv_x(channels=128, blocks=groups[1], strides=1, index=3)
        self.conv4_x = self._make_conv_x(channels=256, blocks=groups[2], strides=1, index=4)
        self.conv5_x = self._make_conv_x(channels=512, blocks=groups[3], strides=1, index=5)
        self.pool2 = nn.AvgPool2d(7)
        patches = 64 if self.block.message == "basic" else 64 * 4
